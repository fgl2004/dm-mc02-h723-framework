...
static void McuInfoApp_WriteLe32(uint8_t *buf, uint32_t value);
static EventManagerPriority_t McuInfoApp_EventToPriority(uint8_t event_id);
...
void McuInfoApp_Run(void)
{
    ...
    /*
     * McuInfoApp no longer forwards an internal event queue.
     * Events are posted directly to EventManager in McuInfoApp_PostEvent().
     */
                         ...
                         uint16_t payload_len)
{
    ...
    event_payload[0] = app_id;
    event_payload[1] = event_id;
    McuInfoApp_WriteLe32(&event_payload[2], PlatformTime_GetMs());
    ...
    {
        memcpy(&event_payload[event_payload_len], payload, copy_len);
        event_payload_len = (uint16_t)(event_payload_len + copy_len);
    }

    ret = EventManager_PostEventEx(event_id,
                                   event_payload,
                                   event_payload_len,
                                   McuInfoApp_EventToPriority(event_id));
    ...
    if (ret == EVENT_MANAGER_QUEUE_FULL)
    {
                              ...
                              uint16_t snapshot_len)
{
    uint8_t i;
    uint16_t copy_len;
    McuInfoSnapshotSlot_t *slot = 0;

    if ((snapshot_len > 0U) && (snapshot_data == 0))
    {
        g_mcu_info_app.stats.invalid_param_count++;
        g_mcu_info_app.stats.last_error = PROTO_ERROR_INVALID_PARAM;
        return MCU_INFO_APP_INVALID_PARAM;
    ...
    if ((snapshot_data != 0) && (copy_len > 0U))
    {
...
void McuInfoApp_PrintStats(void)
{
    ...
    BoardLog_Info("  get_command_stats_count  = %lu\r\n", g_mcu_info_app.stats.get_command_stats_count);
...
{
    char text_buf[96];
    PlatformUartRxSnapshot_t rx_snapshot;
    PlatformUartTxSnapshot_t tx_snapshot;
    int len;
    ...
    PlatformUart_GetRxSnapshot(&rx_snapshot);
    PlatformUart_GetTxSnapshot(&tx_snapshot);

    len = snprintf(text_buf,
                   sizeof(text_buf),
                   "rx=%lu,avail=%u,txdma=%lu,txdone=%lu,err=%lu",
                   (unsigned long)rx_snapshot.rx_bytes,
                   rx_snapshot.rx_ring_available,
                   (unsigned long)tx_snapshot.tx_dma_start_count,
                   (unsigned long)tx_snapshot.tx_dma_done_count,
                   (unsigned long)rx_snapshot.rx_error_count);
...
{
    char text_buf[96];
    const EventManagerStats_t *event_stats;
    int len;
    ...
    event_stats = EventManager_GetStats();
    ...
    len = snprintf(text_buf,
                   sizeof(text_buf),
                   "post=%lu,pop=%lu,drop=%lu,h=%lu,n=%lu,last=0x%02X",
                   (unsigned long)event_stats->post_event_count,
                   (unsigned long)event_stats->event_pop_count,
                   (unsigned long)event_stats->event_drop_count,
                   (unsigned long)event_stats->high_event_post_count,
                   (unsigned long)event_stats->normal_event_post_count,
                   event_stats->last_event_id);
...
static EventManagerPriority_t McuInfoApp_EventToPriority(uint8_t event_id)
{
    switch (event_id)
    {
        case MCU_INFO_EVENT_FAULT:
        case MCU_INFO_EVENT_UART_WARNING:
            return EVENT_MANAGER_PRIORITY_HIGH;

        default:
            return EVENT_MANAGER_PRIORITY_NORMAL;
    }
}
